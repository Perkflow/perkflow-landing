...
const iconMap = {
    ...
    Sparkles,
    Building2,
    Gadget: Watch, // Using Watch as a placeholder for Gadget icon since "Gadget" isn't a direct lucide export, or user meant a specific icon. Let's start with Watch or Smartphone.
    Phone,
};
...
const gadgetProducts = [
    {
        ...
        price: "",
        category: "apple",
        isFocal: true, // Special visual handling if needed
    },
    {
        id: 2,
        name: "Iphone",
...
const featuredGadgets = [
    {
        ...
        name: "iphone 17 Pro",
        price: "1500 €",
        image: "https://images.unsplash.com/photo-1696446701796-da61225697cc?q=80&w=1000&auto=format&fit=crop", // Using 15 Pro placeholder
                    ...
                    <div className="flex-1">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6 mb-12 py-20">
                            {gadgetProducts.map((product) => (
                                <div key={product.id} className="flex flex-col items-center gap-4 group cursor-pointer">
                                    <div className="w-full aspect-square bg-white rounded-[2rem] p-6 shadow-sm hover:shadow-md transition-shadow flex items-center justify-center relative overflow-hidden">
                                        {/* Glow effect for iPhone/focal item if needed, but keeping simple for now */}
                                        {product.name === "Iphone" && (
                        ...
                        <div className="grid md:grid-cols-3 gap-6 mb-8">
                            {featuredGadgets.map((gadget) => (
                                <div
                                    key={gadget.id}
                                    className={cn(
                                        "rounded-[2rem] p-8 relative overflow-hiddenh-[400px] flex flex-col justify-between transition-all hover:scale-[1.02]",
                        ...
                        <div className="grid md:grid-cols-3 gap-6">
                            {secondaryGadgets.map((gadget) => (
                                <div
                                    key={gadget.id}
                                    className="bg-white rounded-[2rem] p-8 h-[380px] flex flex-col justify-between transition-all hover:shadow-lg hover:scale-[1.02]"
                                >
                                    <div>
